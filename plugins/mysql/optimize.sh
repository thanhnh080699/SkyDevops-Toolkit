...
detect_mysql_service() {
    if command -v systemctl >/dev/null 2>&1; then
        if systemctl list-unit-files mysqld.service >/dev/null 2>&1; then
            echo "mysqld"
            return
        fi
...
get_mysql_optimize_file() {
    if [ -d /etc/mysql/mysql.conf.d ]; then
        echo "/etc/mysql/mysql.conf.d/99-optimize_mysql.cnf"
    elif [ -d /etc/mysql/conf.d ]; then
        echo "/etc/mysql/conf.d/99-optimize_mysql.cnf"
    elif [ -d /etc/my.cnf.d ]; then
        echo "/etc/my.cnf.d/99-optimize_mysql.cnf"
    else
        echo "/etc/mysql/mysql.conf.d/99-optimize_mysql.cnf"
    fi
}
...
get_mysql_current() {
    ...
    local value=""
    local files=(
        "/etc/mysql/mysql.conf.d/99-optimize_mysql.cnf"
        "/etc/mysql/mysql.conf.d/mysqld.cnf"
        "/etc/mysql/conf.d/99-optimize_mysql.cnf"
        "/etc/mysql/my.cnf"
        "/etc/my.cnf.d/99-optimize_mysql.cnf"
...
mysql_supports_query_cache() {
    local version
    version=$(mysql -V 2>/dev/null | grep -oE '[0-9]+\.[0-9]+' | head -1)
    if [[ "$version" == 5.* ]]; then
        return 0
    fi
    return 1
...
mysql_supports_option() {
    local option=$1
    local daemon_user_args=()
    if id mysql >/dev/null 2>&1; then
        daemon_user_args=(--user=mysql)
    fi

    command -v mysqld >/dev/null 2>&1 || return 1
    mysqld --verbose --help "${daemon_user_args[@]}" 2>/dev/null | grep -q -- "$option"
}
...
optimize_mysql() {
    ...
    ui_init
    ui_border_top
    ui_title "${BOLD}TỐI ƯU HÓA CẤU HÌNH MYSQL${RESET}"
    ui_border_mid
    ...
    local cur_buffer_pool=$(get_mysql_current "innodb_buffer_pool_size")
    local cur_log_file=$(get_mysql_current "innodb_log_file_size")
    local cur_redo_capacity=$(get_mysql_current "innodb_redo_log_capacity")
    local cur_flush=$(get_mysql_current "innodb_flush_log_at_trx_commit")
    local cur_max_conn=$(get_mysql_current "max_connections")
    local cur_tmp=$(get_mysql_current "tmp_table_size")
    local cur_heap=$(get_mysql_current "max_heap_table_size")
    local cur_thread_cache=$(get_mysql_current "thread_cache_size")
    local cur_key_buffer=$(get_mysql_current "key_buffer_size")
    local cur_query_cache=$(get_mysql_current "query_cache_size")
    local cur_flush_method=$(get_mysql_current "innodb_flush_method")
    local cur_io_capacity=$(get_mysql_current "innodb_io_capacity")
    local cur_io_capacity_max=$(get_mysql_current "innodb_io_capacity_max")
    local cur_read_io_threads=$(get_mysql_current "innodb_read_io_threads")
    local cur_write_io_threads=$(get_mysql_current "innodb_write_io_threads")
    local cur_log_buffer=$(get_mysql_current "innodb_log_buffer_size")
    local cur_table_open_cache=$(get_mysql_current "table_open_cache")
    local cur_table_def_cache=$(get_mysql_current "table_definition_cache")
    local cur_open_files=$(get_mysql_current "open_files_limit")
    local cur_wait_timeout=$(get_mysql_current "wait_timeout")
    local cur_interactive_timeout=$(get_mysql_current "interactive_timeout")
    local cur_connect_timeout=$(get_mysql_current "connect_timeout")
    local cur_slow_log=$(get_mysql_current "slow_query_log")
    local cur_long_query_time=$(get_mysql_current "long_query_time")
    local cur_log_not_using_indexes=$(get_mysql_current "log_queries_not_using_indexes")
    ...
    local sug_redo_capacity="512M"
    [ "$ram_mb" -ge 4096 ] && sug_redo_capacity="1G"
    [ "$ram_mb" -ge 16384 ] && sug_redo_capacity="2G"
    local write_redo_capacity=false
    if mysql_supports_option "innodb-redo-log-capacity"; then
        write_redo_capacity=true
        sug_log_file_size="skip"
    else
        sug_redo_capacity="skip"
    fi
    ...
    local sug_log_not_using_indexes=0
    local sug_query_cache_size="0"
    local write_query_cache=false
    if mysql_supports_query_cache; then
        sug_query_cache_size="64M"
        write_query_cache=true
    else
        sug_query_cache_size="skip"
    fi

    clear
    ui_init
    ui_border_top
    ui_title "${BOLD}CẤU HÌNH MYSQL ĐỀ XUẤT${RESET}"
    ui_border_mid
    ...
    local user_log_file_size="skip"
    local user_redo_capacity="skip"
    if [ "$write_redo_capacity" = true ]; then
        ui_opt_param_prompt "2" "innodb_redo_log_capacity" "$cur_redo_capacity" "$sug_redo_capacity"
        read -r user_redo_capacity
        : ${user_redo_capacity:=$sug_redo_capacity}
    else
    ...
    ui_opt_param_prompt "9" "innodb_flush_method" "$cur_flush_method" "$sug_flush_method"
    read -r user_flush_method
    : ${user_flush_method:=$sug_flush_method}

    ui_opt_param_prompt "10" "innodb_io_capacity" "$cur_io_capacity" "$sug_io_capacity"
    read -r user_io_capacity
    : ${user_io_capacity:=$sug_io_capacity}

    ui_opt_param_prompt "11" "innodb_io_capacity_max" "$cur_io_capacity_max" "$sug_io_capacity_max"
    read -r user_io_capacity_max
    : ${user_io_capacity_max:=$sug_io_capacity_max}

    ui_opt_param_prompt "12" "innodb_read_io_threads" "$cur_read_io_threads" "$sug_io_threads"
    read -r user_read_io_threads
    : ${user_read_io_threads:=$sug_io_threads}

    ui_opt_param_prompt "13" "innodb_write_io_threads" "$cur_write_io_threads" "$sug_io_threads"
    read -r user_write_io_threads
    : ${user_write_io_threads:=$sug_io_threads}

    ui_opt_param_prompt "14" "innodb_log_buffer_size" "$cur_log_buffer" "$sug_log_buffer_size"
    read -r user_log_buffer_size
    : ${user_log_buffer_size:=$sug_log_buffer_size}

    ui_opt_param_prompt "15" "table_open_cache" "$cur_table_open_cache" "$sug_table_open_cache"
    read -r user_table_open_cache
    : ${user_table_open_cache:=$sug_table_open_cache}

    ui_opt_param_prompt "16" "table_definition_cache" "$cur_table_def_cache" "$sug_table_def_cache"
    read -r user_table_def_cache
    : ${user_table_def_cache:=$sug_table_def_cache}

    ui_opt_param_prompt "17" "open_files_limit" "$cur_open_files" "$sug_open_files_limit"
    read -r user_open_files_limit
    : ${user_open_files_limit:=$sug_open_files_limit}

    ui_opt_param_prompt "18" "wait_timeout" "$cur_wait_timeout" "$sug_wait_timeout"
    read -r user_wait_timeout
    : ${user_wait_timeout:=$sug_wait_timeout}

    ui_opt_param_prompt "19" "interactive_timeout" "$cur_interactive_timeout" "$sug_interactive_timeout"
    read -r user_interactive_timeout
    : ${user_interactive_timeout:=$sug_interactive_timeout}

    ui_opt_param_prompt "20" "connect_timeout" "$cur_connect_timeout" "$sug_connect_timeout"
    read -r user_connect_timeout
    : ${user_connect_timeout:=$sug_connect_timeout}

    ui_opt_param_prompt "21" "slow_query_log" "$cur_slow_log" "$sug_slow_log"
    read -r user_slow_log
    : ${user_slow_log:=$sug_slow_log}

    ui_opt_param_prompt "22" "long_query_time" "$cur_long_query_time" "$sug_long_query_time"
    read -r user_long_query_time
    : ${user_long_query_time:=$sug_long_query_time}

    ui_opt_param_prompt "23" "log_queries_not_using_indexes" "$cur_log_not_using_indexes" "$sug_log_not_using_indexes"
    read -r user_log_not_using_indexes
    : ${user_log_not_using_indexes:=$sug_log_not_using_indexes}

    local user_query_cache_size=""
    if [ "$write_query_cache" = true ]; then
        ui_opt_param_prompt "24" "query_cache_size" "$cur_query_cache" "$sug_query_cache_size"
        read -r user_query_cache_size
        : ${user_query_cache_size:=$sug_query_cache_size}
    else
        user_query_cache_size="skip"
    fi

    clear
    ui_init
    ui_border_top
    ui_title "${BOLD}XÁC NHẬN THAY ĐỔI MYSQL${RESET}"
    ui_border_mid
    ...
    ui_line "$(printf "%-35s %-18s %-18s" "innodb_redo_log_capacity" "$cur_redo_capacity" "$user_redo_capacity")"
    ...
    ui_line "$(printf "%-35s %-18s %-18s" "log_queries_not_using_indexes" "$cur_log_not_using_indexes" "$user_log_not_using_indexes")"
    ui_line "$(printf "%-35s %-18s %-18s" "query_cache_size" "$cur_query_cache" "$user_query_cache_size")"
    ui_empty
    ui_line "${YELLOW}Cấu hình sẽ được ghi vào include 99-optimize_mysql.cnf và backup nếu file đã tồn tại.${RESET}"
    ...
    local optimize_file
    optimize_file=$(get_mysql_optimize_file)
    local optimize_dir
...
log_queries_not_using_indexes = $user_log_not_using_indexes
EOF
        if [ "$write_redo_capacity" = true ]; then
            echo "innodb_redo_log_capacity = $user_redo_capacity" >> "$tmp_file"
        else
            echo "innodb_log_file_size = $user_log_file_size" >> "$tmp_file"
        fi
        if [ "$write_query_cache" = true ]; then
            echo "query_cache_size = $user_query_cache_size" >> "$tmp_file"
        fi
        ...
        simulate_progress "Đang ghi include MySQL 99-optimize_mysql.cnf"
        simulate_progress "Đang restart MySQL để áp dụng cấu hình"
    fi

    echo -e "\n  ${GREEN}✔ Hoàn tất quy trình tối ưu MySQL!${RESET}"
    pause_before_menu
}
